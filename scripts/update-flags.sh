...
echo ""
echo "Next step: enrich the new snapshot with description + tokenEffect + internal."
echo "  1. node scripts/extract-contexts.mjs --target $NEW_FILE"
echo "  2. node scripts/slice-batches.mjs --target $NEW_FILE --contexts flags/.contexts/$NEW_VERSION.json --batches 5"
echo "  3. In Claude Code, dispatch 5 general-purpose subagents to enrich each batch"
echo "     (each writes flags/.enrichment/batch-N.json). See the feat/flag-enrichment PR for the prompt template."
echo "  4. node scripts/merge-enrichment.mjs --in flags/.enrichment/batch-1.json ... --out flags/.enrichment/$NEW_VERSION.json"
echo "  5. node scripts/apply-enrichment.mjs --target $NEW_FILE --enrichment flags/.enrichment/$NEW_VERSION.json"
